...
class SignupFlowTests(unittest.TestCase):
    ...
    def test_patchright_captcha_keeps_result_updata1_flow_with_english_labels(self):
        ...
        self.assertIn("frame_any = frame1.frame_locator('iframe')", source)
        self.assertIn("frame_any.locator(challenge_selector)", source)
        self.assertIn("wait_first_visible(locators, timeout=12000)", source)
